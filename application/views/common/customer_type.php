...
<div class="row">
	<div class="col-xl-12">
		<div class="ibox collapsible-box">
			<div class="ibox-head">
				<div class="ibox-title">LOẠI KHÁCH HÀNG</div>
				...
				<div class="col-md-12 col-sm-12 col-xs-12 table-responsive">
					<div id="tablecontent">
						<table id="contenttable" class="table table-striped display nowrap" cellspacing="0" style="width: 99.8%">
							...
							<?php if(count($customertype) > 0) {$i = 1; ?>
...
<script type="text/javascript">
	$(document).ready(function () {		
		var _columns 		= ["STT", "rowguid", "CusTypeID", "CusTypeName"],
			tbl 			= $('#contenttable'),
			customertype 	= {},
			parentMenuList 	= {};

		/* Load data from Customer Type Table */
		<?php if(isset($customertype)){?>
		...
		var dataTbl = tbl.DataTable({
			...
			columnDefs: [
				...
				{ className: "text-center", targets: _columns.getIndexs(["CusTypeID", "CusTypeName"]) },
        ...
        function saveData(){        	
			...
			for (i = 0; i < newData.length; i++){
				for (j = 0; j < customertype.length; j++){
					if (newData[i]['CusTypeID'] == customertype[j]['CusTypeID']){
						toastr['error']("Đã tồn tại Mã loại khách hàng: " + newData[i]['CusTypeID']);
						return;
					}
				}
			}
		...
		function postSave(formData){
			...
			$.ajax({
                url: "<?=site_url(md5('Common') . '/' . md5('cmCustomerType'));?>",
                dataType: 'json',
                data: formData,
                type: 'POST',
                success: function (datas) {
                    ...
                    if(formData.action == 'edit'){
		                ...
		                if(data.error && data.error.length > 0){
		                	for (var i = 0; i < data.error.length; i++) {
				                ...
				                var	str 	= data.error[i].split(':')[1].trim(),
				                	...
				                	cell 	= tbl.find("tbody tr:eq(" + cRow + ") td:eq("+ _columns.indexOf("CusTypeID") +")");			    
                    	...
                    	socket.emit('DMDC_updateSavedData', JSON.stringify(tblData));
        ...
        function postDel(rows){
			...
			var delCustomerType = rows.map(p=>p[_columns.indexOf("CusTypeID")]);
			var delBtn = $('#delete');
			delBtn.button('loading');

			var formdata = {
				'action': 'delete',
				'data': delCustomerType
			};
			$.ajax({
				url: "<?=site_url(md5('Common') . '/' . md5('cmCustomerType'));?>",
				dataType: 'json',
				...
				success: function (output) {
	                ...
	                if(data.success && data.success.length > 0){
	                	...
	                	for (var i = 0; i < data.success.length; i++) {
	                		...
	                		var indexes = tbl.filterRowIndexes( _columns.indexOf( "CusTypeID" ), valueID);
	                	...
	                	/* Socket real-time */
                    	socket.emit('DMDC_updateDeletedData', JSON.stringify(delList));
	                }
		...
		socket.on('DMDC_updateSavedData', function(data){
			var tblData = JSON.parse(data),
				rows = [];

			for (i = 0; i < tblData.length; i++){
				var r = [];
				$.each(_columns, function(idx, colname){
					r.push(tblData[i][colname]);
				});
				rows.push(r);
			}
		...
		socket.on('DMDC_updateDeletedData', function(data){
			var delList = JSON.parse(data);

			for (k = 0; k < delList.length; k++){
				var indexes = tbl.filterRowIndexes( _columns.indexOf( "CusTypeID" ), delList[k]);

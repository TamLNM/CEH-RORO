...
<div class="row">
	<div class="col-xl-12">
		<div class="ibox collapsible-box">
			...
			<div class="ibox-head">
				<div class="ibox-title">TỶ GIÁ</div>
				...
				<div class="col-md-12 col-sm-12 col-xs-12 table-responsive">
					<table id="contenttable" class="table table-striped display nowrap" cellspacing="0" style="width: 99.5%">
						...
						<tr>
							...
							<th col-name="RateID">Mã tỷ giá</th>
							<th col-name="BrandID">Hãng xe</th>
							<th col-name="ExchangeRate">Tỷ giá</th>
						</tr>
						</thead>
						<tbody>
							<?php if(count($rateList) > 0) {
								  		$i = 1; ?>
								<?php foreach($rateList as $item){  ?>									
									<tr>
										<td col-name="STT"><?=$i;?></td>
										<td col-name="RateID"><?=$item['RateID'];?></td>
										<td col-name="BrandID"><input class='hiden-input' value="<?=$item['BrandID'];?>"> <?=$item['BrandName'];?></td>
										<td col-name="ExchangeRate"><?=$item['ExchangeRate'];?></td>		
									</tr>
									<?php $i++; }  ?>
							<?php } ?>			
						</tbody>
...
<script type="text/javascript">
	$(document).ready(function () {
		var tbl 			= $('#contenttable'),
			_columns 		= ['STT', 'RateID', 'BrandID', 'ExchangeRate'],
			rateList		= {},
			carBrandList	= {};

		<?php if(isset($rateList) && count($rateList) > 0){?>
			rateList = <?=json_encode($rateList);?>;
		<?php } ?>
		...
		var dataTbl = tbl.newDataTable({
			scrollY: '65vh',
			columnDefs: [
				{ type: "num", className: "text-center", targets: _columns.getIndexs(['STT', 'ExchangeRate']) },
				{ className: "text-center", targets: _columns.getIndexs(['RateID', 'BrandID'])},
			],
        ...
        function saveData(){        	
			...
			for (i = 0; i < newData.length; i++){
				if (!newData[i]['RateID']){
					toastr['error']("Vui lòng nhập Mã tỷ giá!");
					return;
				}
				else{
					for (j = 0; j < rateList.length; j++){
						if (newData[i]['RateID'] == rateList[j]['RateID']){
							toastr['error']("Đã tồn tại Mã tỷ giá: " + newData[i]['RateID']);
							return;
						}
					}	
				}		
				...
				if (!newData[i]['BrandID']){
					toastr['error']("Vui lòng chọn Hãng xe cho Mã tỷ giá: " + newData[i]['RateID'] + "!");
					return;
				}

				if (!newData[i]['ExchangeRate']){
					toastr['error']("Vui lòng nhập Tỷ giá cho Mã tỷ giá: " + newData[i]['RateID'] + "!");
					return;
				}

				if (!($.isNumeric(newData[i]['ExchangeRate']))){
					toastr['error']("Vui lòng nhập Tỷ giá hợp lệ cho Mã tỷ giá: " + newData[i]['RateID'] + ".");
					return;
				}
			}
		...
		function postSave(formData){
			...
			$.ajax({
                url: "<?=site_url(md5('Common') . '/' . md5('cmRate'));?>",
                dataType: 'json',
                data: formData,
                type: 'POST',
		...
		function postDel(data){
			var delData = data.map(p=>p[_columns.indexOf("RateID")]);

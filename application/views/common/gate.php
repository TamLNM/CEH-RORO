				...
				<div class="col-md-12 col-sm-12 col-xs-12 table-responsive">
					<table id="contenttable" class="table table-striped display nowrap" cellspacing="0" style="width: 99.5%">
						...
						<tr>
							...
							<th col-name="GateID">Mã cổng</th>
							<th col-name="GateName">Tên cổng</th>
							...
							<?php if(count($gateList) > 0) {
								...
								<?php foreach($gateList as $item){  ?>									
									<tr>
										...
										<td col-name="GateID"><?=$item['GateID'];?></td>
										<td col-name="GateName"><?=$item['GateName'];?></td>	
										<td col-name="InOut">
											<?php if ($item['InOut'] == 1) {?>
												<input class="hiden-input" value="<?=$item['InOut'];?>">Vào</td>	
											<?php } else{ ?>
												<input class="hiden-input" value="<?=$item['InOut'];?>">Ra</td>	
											<?php }?>
										</td>			
										<td col-name="ClassID">
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		var tbl 			= $('#contenttable'),
			_columns 		= ['STT', 'rowguid', 'GateID', 'GateName', 'InOut', 'ClassID'],
			ioList	 		= {1: 'Vào', 2: 'Ra'},
			gateList		= {},
			classList		= {},
			parentMenuList 	= {};
		...
		var dataTbl = tbl.newDataTable({
			...
			columnDefs: [
				...
				{ className: "text-center", targets: _columns.getIndexs(['GateID', 'GateName', 'InOut', 'ClassID'])},
        ...
        function saveData(){        	
			...
			for (i = 0; i < newData.length; i++){
				...
				if (!newData[i]['GateID']){
					toastr['error']("Vui lòng nhập Mã quốc gia!");
					return;
				}
				else{
					for (j = 0; j < gateList.length; j++){
						if (newData[i]['GateID'] == gateList[j]['GateID']){
							toastr['error']("Đã tồn tại Mã quốc gia: " + newData[i]['GateID']);
							return;
						}
					}	
				}		

				if (!newData[i]['GateName']){
					toastr['error']("Vui lòng nhập Tên cho Mã quốc gia: " + newData[i]['GateID'] + "!");
					return;
				}

				if (!newData[i]['InOut']){
					toastr['error']("Vui lòng nhập Quốc kỳ cho Mã quốc gia: " + newData[i]['GateID'] + "!");
					return;
				}

				if (!newData[i]['ClassID']){
					toastr['error']("Vui lòng nhập Quốc kỳ cho Mã quốc gia: " + newData[i]['ClassID'] + "!");
					return;
				}
			}
		...
		function postSave(formData){
			...
			$.ajax({
                url: "<?=site_url(md5('Common') . '/' . md5('cmGate'));?>",
                dataType: 'json',
                data: formData,
                type: 'POST',
		                ...
		                if(data.error && data.error.length > 0){
		                	for (var i = 0; i < data.error.length; i++) {
				                ...
				                var	str 	= data.error[i].split(':')[1].trim(),
				                	...
				                	cell 	= tbl.find("tbody tr:eq(" + cRow + ") td:eq("+ _columns.indexOf("GateID") +")");			    
		...
		function postDel(data){
			var delData = data.map(p=>p[_columns.indexOf("GateID")]);

...
<div class="row">
	<div class="col-xl-12">
		<div class="ibox collapsible-box">
			...
			<div class="ibox-head">
				<div class="ibox-title">LOẠI HÌNH VẬN CHUYỂN</div>
				...
				<div class="col-md-12 col-sm-12 col-xs-12 table-responsive">
					<table id="contenttable" class="table table-striped display nowrap" cellspacing="0" style="width: 99.5%">
						...
						<tr>
							...
							<th col-name="TransitID">Mã loại hình</th>
							<th col-name="TransitName">Tên loại hình</th>
						</tr>
						</thead>
						<tbody>
							<?php if(count($transitList) > 0) {
								  		$i = 1; ?>
								<?php foreach($transitList as $item){  ?>									
									<tr>
										<td col-name="STT"><?=$i;?></td>
										<td col-name="TransitID"><?=$item['TransitID'];?></td>
										<td col-name="TransitName"><?=$item['TransitName'];?></td>		
									</tr>
									<?php $i++; }  ?>
							<?php } ?>			
						</tbody>
...
<script type="text/javascript">
	$(document).ready(function () {
		var tbl 			= $('#contenttable'),
			_columns 		= ['STT', 'TransitID', 'TransitName'],
			transitList		= {},
			parentMenuList 	= {};
		...
		var dataTbl = tbl.newDataTable({
			...
			columnDefs: [
				...
				{ className: "text-center", targets: _columns.getIndexs(['TransitID', 'TransitName'])},
			],
        ...
        function saveData(){        	
			...
			for (i = 0; i < newData.length; i++){
				if (!newData[i]['TransitID']){
					toastr['error']("Vui lòng nhập Mã loại hình!");
					return;
				}
				else{
					for (j = 0; j < transitList.length; j++){
						if (newData[i]['TransitID'] == transitList[j]['TransitID']){
							toastr['error']("Đã tồn tại Mã loại hình: " + newData[i]['TransitID']);
							return;
						}
					}	
				}		
				...
				if (!newData[i]['TransitName']){
					toastr['error']("Vui lòng nhập Tên cho Mã loại hình: " + newData[i]['TransitName'] + "!");
					return;
				}
			}
		...
		function postSave(formData){
			...
			$.ajax({
                url: "<?=site_url(md5('Common') . '/' . md5('cmTransit'));?>",
                dataType: 'json',
                data: formData,
                type: 'POST',
		...
		function postDel(data){
			...
			var delData = data.map(p=>p[_columns.indexOf("TransitID")]);
                ...
                success: function (data) {
	                ...
	                if(data.success && data.success.length > 0){
	                	for (var i = 0; i < data.success.length; i++) {
	                		...
	                		var indexes = tbl.filterRowIndexes( _columns.indexOf( "TransitID" ), valueID);

					...
					<div id="tablecontent">						
						<table id="contenttable" class="table table-striped display nowrap" cellspacing="0" style="width: 99.8%">						
							...
							<tr>
								...
								<th col-name="rowguid" class="hiden-input"></th>
								<th col-name="BerthID">Mã bến</th>
								<th col-name="DeepHeight">Độ sâu</th>
								<th col-name="Capacity">Trọng tải</th>
								<th col-name="PosFrom">Từ mét</th>
								<th col-name="PosTo">Đến mét</th>
							</tr>
							</thead>
							<tbody>
							<?php if(count($berthList) > 0) {$i = 1; ?>
								<?php foreach($berthList as $item) {  ?>
									<tr>
										<td style="text-align: center"><?=$i;?></td>
										<td col-name="rowguid"><?=$item['rowguid'];?></td>
										<td col-name="BerthID"><?= $item['BerthID']; ?></td>
										<td col-name="DeepHeight"><?= $item['DeepHeight']; ?></td>
										<td col-name="Capacity"><?= $item['Capacity']; ?></td>
										<td col-name="PosFrom"><?= $item['PosFrom']; ?></td>
										<td col-name="PosTo"><?= $item['PosTo']; ?></td>
									</tr>
								<?php $i++; }  ?>
							<?php } ?>
							</tbody>
...
<script type="text/javascript">
	$(document).ready(function (){
		var _columns 	= ["STT", "rowguid", "BerthID", "DeepHeight", "Capacity", "PosFrom", "PosTo"],
			tbl 		= $('#contenttable'),
			berthList 	= {},
			parentMenuList 	= {};
		...
		var dataTbl = tbl.newDataTable({
			...
			columnDefs: [
				...
				{ className: "text-center", targets: _columns.getIndexs(["BerthID",  "DeepHeight", "Capacity", "PosFrom", "PosTo"]) },
        ...
        $('#save').on('click', function(){
            if(tbl.DataTable().rows( '.addnew, .editing' ).data().toArray().length == 0){
				...
				for (i = 0; i < newData.length; i++){
					for (j = 0; j < berthList.length; j++){
						if (newData[i]['BerthID'] == berthList[j]['BerthID']){
		...
		function postSave(formData){
			...
			$.ajax({
                url: "<?=site_url(md5('Common') . '/' . md5('cmBerth'));?>",
                dataType: 'json',
                data: formData,
                type: 'POST',
		                ...
		                if(data.error && data.error.length > 0){
		                	for (var i = 0; i < data.error.length; i++) {
				                ...
				                var	str 	= data.error[i].split(':')[1].trim(),
				                	...
				                	cell 	= tbl.find("tbody tr:eq(" + cRow + ") td:eq("+ _columns.indexOf("BerthID") +")");
        ...
        function postDel(rows){
			...
			var delWorker = rows.map(p=>p[_columns.indexOf("BerthID")]);
				...
				success: function (output) {
	                ...
	                if(data.success && data.success.length > 0){
	                	for (var i = 0; i < data.success.length; i++) {
	                		...
	                		var indexes = tbl.filterRowIndexes( _columns.indexOf( "BerthID" ), valueID);
	                		tbl.DataTable().rows( indexes ).remove().draw( false );
	                		tbl.updateSTT( _columns.indexOf( "STT" ) );
	                		toastr["success"]( data.success[i] );

	                		for (j = 0; j < berthList.length; j++){
	                			if (berthList[j]['BerthID'] == valueID){
	                				berthList.splice(j, 1);
	                			}
	                		}
	                	}
	                }

...
<div class="row">
	<div class="col-xl-12">
		<div class="ibox collapsible-box">
			...
			<div class="ibox-head">
				<div class="ibox-title">VÙNG PHỤ</div>
				...
				<div class="col-md-12 col-sm-12 col-xs-12 table-responsive">
					<table id="contenttable" class="table table-striped display nowrap" cellspacing="0" style="width: 99.5%">
						...
						<tr>
							...
							<th col-name="Area">Mã vùng</th>
							<th col-name="AreaName">Tên vùng</th>
							<th col-name="Capacity">Sức chứa</th>
						</tr>
						</thead>
						<tbody>
							<?php if(count($areaList) > 0) {
								...
								<?php foreach($areaList as $item){  ?>									
									<tr>
										...
										<td col-name="Area"><?=$item['Area'];?></td>
										<td col-name="AreaName"><?=$item['AreaName'];?></td>		
										<td col-name="Capacity"><?=$item['Capacity'];?></td>		
									</tr>
									<?php $i++; }  ?>
							<?php } ?>			
						</tbody>
...
<script type="text/javascript">
	$(document).ready(function () {
		var tbl 			= $('#contenttable'),
			_columns 		= ['STT', 'rowguid', 'Area', 'AreaName', 'Capacity'],
			areaList		= {},
			parentMenuList 	= {};

		<?php if(isset($areaList) && count($areaList) >= 0){?>
		...
		var dataTbl = tbl.newDataTable({
			...
			columnDefs: [
				...
				{ className: "text-center", targets: _columns.getIndexs(['Area', 'AreaName', 'Capacity'])},
			...
			/*
            keys: {
				columns: ':not(:eq(' + _columns.indexOf("STT") + '), :eq(' + _columns.indexOf("Area") + '))'
			},
			*/
			keys: true,
            autoFill: {
		...
		/*
		var checkCol 	= parseInt(_columns.indexOf("Area")) + 1,
			value 		= '';
		$(document).on("dblclick", "#contenttable tbody td:nth-child(" + checkCol  + ")",  function(e){
			var inp 	= $(e.target),
	       	...
	       	if(!crRow.hasClass("")){
	       		...
	       		value = inp[0]['innerHTML'];
	       		//cell = tbl.find("tbody tr:eq(" + crRow + ") td:eq("+ crCol +")");
		...
		*/
		...
		$("#apply-add-row").on("click", function(){
			...
			if (numRows == 1){
				...
				for (i = 0; i < rowsExist; i++){
					cell = tbl.find("tbody tr:eq(" + i + ") td:eq(1)");
		...
		$('#save').on('click', function(){
            if(tbl.DataTable().rows( '.addnew, .editing' ).data().toArray().length == 0){
				...
				for (i = 0; i < newData.length; i++){
					if (!newData[i]['Area']){
						toastr['error']("Vui lòng nhập Mã vùng!");
						return;
					}
					else{
						for (j = 0; j < areaList.length; j++){
							if (newData[i]['Area'] == areaList[j]['Area']){
								toastr['error']("Đã tồn tại Mã vùng: " + newData[i]['Area']);
								return;
							}
						}	
					}		

					if (!newData[i]['AreaName']){
						toastr['error']("Vui lòng nhập Tên cho Mã vùng: " + newData[i]['AreaName'] + "!");
						return;
					}

					if (!newData[i]['Capacity']){
						toastr['error']("Vui lòng nhập Sức chứa cho Mã vùng: " + newData[i]['AreaName'] + "!");
						return;
					}

					if (!($.isNumeric(newData[i]['Capacity']))){
						toastr['error']("Vui lòng nhập Sức chứa hợp lệ cho Mã vùng: " + newData[i]['AreaName'] + ".");
						return;
					}
				}
		...
		function postSave(formData){
			...
			$.ajax({
                url: "<?=site_url(md5('Common') . '/' . md5('cmArea'));?>",
                dataType: 'json',
                data: formData,
                type: 'POST',
		...
		function postDel(data){
			...
			var delData = data.map(p=>p[_columns.indexOf("Area")]);
                ...
                success: function (data) {
	                ...
	                if(data.success && data.success.length > 0){
	                	for (var i = 0; i < data.success.length; i++) {
	                		...
	                		var indexes = tbl.filterRowIndexes( _columns.indexOf( "Area" ), valueID);
	                		tbl.DataTable().rows( indexes ).remove().draw( false );
	                		tbl.updateSTT( _columns.indexOf( "STT" ) );
	                		toastr["success"]( data.success[i] );

	                		for (j = 0; j < areaList.length; j++){
	                			if (areaList[j]['Area'] == valueID){
	                				areaList.splice(j, 1);
	                			}
	                		}
	                	}

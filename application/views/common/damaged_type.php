				...
				<div class="col-md-12 col-sm-12 col-xs-12 table-responsive">
					<div id="tablecontent">
						<table id="contenttable" class="table table-striped display nowrap" cellspacing="0" style="width: 99.8%">
							...
							<tbody>
							<?php if(count($damagedTypeList) > 0) {$i = 1; ?>
								<?php foreach($damagedTypeList as $item) {  ?>
									<tr>
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		var _columns 		= ["STT", "rowguid", "DamagedTypeID", "DamagedTypeName"],
			tbl 			= $('#contenttable'),
			damagedTypeList = {},
			parentMenuList 	= {};

		/* Load data from Damaged Type Table */
		...
		var dataTbl = tbl.DataTable({
			...
			columnDefs: [
				...
				{ className: "text-center", targets: _columns.getIndexs(["DamagedTypeID", "DamagedTypeName"]) },
        ...
        function saveData(){        	
			...
			for (i = 0; i < newData.length; i++){
				for (j = 0; j < damagedTypeList.length; j++){
					if (newData[i]['DamagedTypeID'] == damagedTypeList[j]['DamagedTypeID']){
						toastr['error']("Đã tồn tại mã loại hư hỏng: " + newData[i]['DamagedTypeID']);
						return;
					}
				}
			}

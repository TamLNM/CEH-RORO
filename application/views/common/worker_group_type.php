...
<div class="row">
	<div class="col-xl-12">
		<div class="ibox collapsible-box">
			<div class="ibox-head">
				<div class="ibox-title">LOẠI TỔ ĐỘI CÔNG NHÂN</div>			
					...
					<div id="tablecontent">						
						<table id="contenttable" class="table table-striped display nowrap" cellspacing="0" style="width: 99.8%">						
							...
							<tr>
								...
								<th col-name="WorkerGroupType" >Mã loại</th>
								<th col-name="WorkerGroupTypeName">Tên loại</th>
							</tr>
							</thead>
							<tbody>
							<?php if(count($workerGroupTypeList) > 0) {$i = 1; ?>
								<?php foreach($workerGroupTypeList as $item) {  ?>
									<tr>
										<td style="text-align: center"><?=$i;?></td>
										<td><?=$item['WorkerGroupType'];?></td>
										<td><?=$item['WorkerGroupTypeName'];?></td>
									</tr>
								<?php $i++; }  ?>
							<?php } ?>
							</tbody>
...
<!--edit UnitID = WorkerGroupType, UnitName = WorkerGroupTypeName-->
<script type="text/javascript">
	$(document).ready(function () {
		var _columns 	= ["STT", "WorkerGroupType", "WorkerGroupTypeName"],
			tbl 		= $('#contenttable'),
			workerGroupTypeList = {},
			parentMenuList 	= {};
		...
		var dataTbl = tbl.DataTable({
			...
			columnDefs: [
				...
				{ className: "text-center", targets: _columns.getIndexs(["WorkerGroupType", "WorkerGroupTypeName"]) },
			],
        ...
        function saveData(){        	
			...
			for (i = 0; i < newData.length; i++){
				for (j = 0; j < workerGroupTypeList.length; j++){
					if (newData[i]['WorkerGroupType'] == workerGroupTypeList[j]['WorkerGroupType']){
						toastr['error']("Đã tồn tại Mã tổ đội công nhân " + newData[i]['WorkerGroupType']);
						return;
					}
				}
			}
		...
		function postSave(formData){
			...
			$.ajax({
                url: "<?=site_url(md5('Common') . '/' . md5('cmWorkerGroupType'));?>",
                dataType: 'json',
                data: formData,
                type: 'POST',
        ...
        function postDel(rows){
			...
			var delWorker = rows.map(p=>p[_columns.indexOf("WorkerGroupType")]);
			var delBtn = $('#delete');
				...
				success: function (output) {
	                ...
	                if(data.success && data.success.length > 0){
	                	for (var i = 0; i < data.success.length; i++) {
	                		...
	                		var indexes = tbl.filterRowIndexes( _columns.indexOf( "WorkerGroupType" ), deletedUnitCode);

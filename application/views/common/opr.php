			...
			<div class="row ibox-footer border-top-0">
				<div class="col-md-12 col-sm-12 col-xs-12 table-responsive">
					<div style="text-align: right;">
						<label class="col-form-label" style="font-style: italic;">* Nếu có nhiều địa chỉ Email: các Email cách nhau bởi dấu ;</label>
					</div>
...
<script type="text/javascript">
	$(document).ready(function () {
		var tbl 			= $('#contenttable'),
			_columns 		= ['STT', 'OprID', 'OprName', 'Email'],
			oprList			= {},
			parentMenuList 	= {};
		...
		var dataTbl = tbl.newDataTable({
			...
			columnDefs: [
				...
				{ className: "text-center", targets: _columns.getIndexs(['OprID', 'OprName'])},
			],
        ...
        function saveData(){        	
			...
			for (i = 0; i < newData.length; i++){
				if (!newData[i]['OprID']){
					toastr['error']("Vui lòng nhập Mã hãng!");
					return;
				}
				else{
					for (j = 0; j < oprList.length; j++){
						if (newData[i]['OprID'] == oprList[j]['OprID']){
							toastr['error']("Đã tồn tại Mã hãng: " + newData[i]['OprID']);
							return;
						}
					}	
				}		

				if (!newData[i]['OprName']){
					toastr['error']("Vui lòng nhập tên cho Mã hãng: " + newData[i]['OprID'] + "!");
					return;
				}
			}
		...
		function postSave(formData){
			...
			$.ajax({
                url: "<?=site_url(md5('Common') . '/' . md5('cmOpr'));?>",
                dataType: 'json',
                data: formData,
                type: 'POST',
		...
		function postDel(data){
			var delData = data.map(p=>p[_columns.indexOf("OprID")]);

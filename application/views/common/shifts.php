...
<div class="row">
	<div class="col-xl-12">
		<div class="ibox collapsible-box">
			...
			<div class="ibox-head">
				<div class="ibox-title">CA SẢN XUẤT</div>
				...
				<div class="col-md-12 col-sm-12 col-xs-12 table-responsive">
					<table id="contenttable" class="table table-striped display nowrap" cellspacing="0" style="width: 99.5%">
						...
						<tr>
							...
							<th col-name="ShiftID">Mã ca</th>
							<th col-name="ShiftName">Tên ca</th>
							<th class="data-type-numeric" col-name="FromTime">Thời gian bắt đầu</th>
							<th class="data-type-numeric" col-name="ToTime">Thời gian kết thúc</th>		
						</tr>
						</thead>
						<tbody>
							<?php if(count($shiftList) > 0) {
								...
								<?php foreach($shiftList as $item){  ?>									
									<tr>
										...
										<td col-name="ShiftID"><?=$item['ShiftID'];?></td>
										<td col-name="ShiftName"><?=$item['ShiftName'];?></td>	
										<td class="data-type-numeric" col-name="FromTime"><?=$item['FromTime'];?></td>		
										<td class="data-type-numeric" col-name="ToTime"><?=$item['ToTime'];?></td>		
									</tr>
									<?php $i++; }  ?>
							<?php } ?>					
						</tbody>
...
<script type="text/javascript">
	$(document).ready(function () {
		var tbl 		= $('#contenttable'),
			_columns 	= ['STT', 'ShiftID', 'ShiftName', 'FromTime', 'ToTime'],
			shiftList	= {},
			parentMenuList 	= {};

		<?php if(isset($shiftList) && count($shiftList) >= 0){?>
			shiftList = <?=json_encode($shiftList);?>;
		<?php } ?>
		...
		var dataTbl = tbl.newDataTable({
			...
			columnDefs: [
				...
				{ className: "text-center", targets: _columns.indexOf('ShiftID')},
				{ className: "text-center", targets: _columns.indexOf('ShiftName')},
				{ type: "num", className: "text-center", targets: _columns.indexOf('FromTime')},
				{ type: "num", className: "text-center", targets: _columns.indexOf('ToTime')},
			],
		...
		$("#save").on('click', function(){
			if(tbl.DataTable().rows( '.addnew, .editing' ).data().toArray().length == 0){
            	...
            	for (i = 0; i < newData.length; i++){
            		var checkShiftID	= newData[i]['ShiftID'],
					checkShiftName     	= newData[i]['ShiftName'],
					checkFromTime   	= newData[i]['FromTime'],
					checkToTime		   	= newData[i]['ToTime'];

					if(!checkShiftID){
						toastr["error"]("Vui lòng nhập Mã ca!");
						check = false;
						return ;
					}
					else{
						for (j = 0; j < shiftList.length; j++){
							if (checkShiftID == shiftList[j]['ShiftID']){
								toastr['error']("Đã tồn tại Mã ca sản xuất: " + checkShiftID);
								check = false;
								return ;
							}
						}	
					...
					if(!checkShiftName){
						toastr["error"]("Vui lòng nhập Tên ca!");
						check = false;
						return ;
					}

					if(!checkFromTime){
						toastr["error"]("Vui lòng nhập Thời gian bắt đầu!");
						check = false;
						return ;
					}

					if(!checkToTime){
						toastr["error"]("Vui lòng nhập Thời gian kết thúc!");
						check = false;
						return ;
					}
					...
					if (checkFromTime > checkToTime){
						toastr["error"]("Thời gian bắt đầu không đượt vượt quá thời gian kết thúc!");
						check = false;
						return ;					
					}
				}
		...
		function postSave(formData){
			...
			$.ajax({
                url: "<?=site_url(md5('Common') . '/' . md5('cmShifts'));?>",
                dataType: 'json',
                data: formData,
                type: 'POST',
		...
		function postDel(data){
			var delPortID = data.map(p=>p[_columns.indexOf("ShiftID")]);

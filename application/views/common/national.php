...
<div class="row">
	<div class="col-xl-12">
		<div class="ibox collapsible-box">
			...
			<div class="ibox-head">
				<div class="ibox-title">QUỐC GIA</div>
							...
							<?php if(count($nationList) > 0) {
								...
								<?php foreach($nationList as $item){  ?>									
									<tr>
										...
										<td col-name="NationName"><?=$item['NationName'];?></td>		
										<td col-name="Flag"><?=$item['Flag'];?></td>		
									</tr>
									<?php $i++; }  ?>
							<?php } ?>			
						</tbody>
...
<script type="text/javascript">
	$(document).ready(function () {
		var tbl 			= $('#contenttable'),
			_columns 		= ['STT', 'NationID', 'NationName', 'Flag'],
			nationList		= {},
			parentMenuList 	= {};
		...
		var dataTbl = tbl.newDataTable({
			...
			columnDefs: [
				...
				{ className: "text-center", targets: _columns.getIndexs(['NationID', 'NationName', 'Flag'])},
			],
        ...
        function saveData(){        	
			...
			for (i = 0; i < newData.length; i++){
				if (!newData[i]['NationID']){
					toastr['error']("Vui lòng nhập Mã quốc gia!");
					return;
				}
				else{
					for (j = 0; j < nationList.length; j++){
						if (newData[i]['NationID'] == nationList[j]['NationID']){
							toastr['error']("Đã tồn tại Mã quốc gia: " + newData[i]['NationID']);
							return;
						}
					}	
				}		

				if (!newData[i]['NationName']){
					toastr['error']("Vui lòng nhập Tên cho Mã quốc gia: " + newData[i]['NationID'] + "!");
					return;
				}
			}
		...
		function postSave(formData){
			...
			$.ajax({
                url: "<?=site_url(md5('Common') . '/' . md5('cmNational'));?>",
                dataType: 'json',
                data: formData,
                type: 'POST',
		...
		function postDel(data){
			...
			var delData = data.map(p=>p[_columns.indexOf("NationID")]);

				...
				<div class="col-md-12 col-sm-12 col-xs-12 table-responsive">
					<table id="contenttable" class="table table-striped display nowrap" cellspacing="0" style="width: 99.5%">
						...
						<tr>
							...
							<th col-name="GroupID">Mã nhóm</th>
							...
							<?php if(count($sysMenuList) > 0) {
								...
								<?php foreach($sysMenuList as $item){  ?>									
									<tr>
										...
										<td col-name="GroupID"><?=$item['GroupID'];?></td>
										<td col-name="GroupName"><?=$item['GroupName'];?></td>				
									</tr>
									<?php $i++; }  ?>
							<?php } ?>			
						</tbody>
...
<script type="text/javascript">
	$(document).ready(function () {
		var _columns 	= ["STT", "GroupID", "GroupName"],
			tbl 	 	= $("#contenttable"),
			parentMenuList = {},
			sysMenuList = {};
		...
		var dataTbl = tbl.newDataTable({
			...
			columnDefs: [
				...
				{ className: "text-center", targets: _columns.getIndexs(['GroupID', 'GroupName'])},
			],
            ...
            if(tbl.DataTable().rows( '.addnew, .editing' ).data().toArray().length == 0){
				...
				for (i = 0; i < newData.length; i++){
					if (!newData[i]['GroupID']){
						...
						for (j = 0; j < sysMenuList.length; j++){
							if (newData[i]['GroupID'] == sysMenuList[j]['GroupID']){
		...
		function postSave(formData){
			...
			$.ajax({
                url: "<?=site_url(md5('Users_Management') . '/' . md5('umSysGroup'));?>",
                dataType: 'json',
                data: formData,
                type: 'POST',
		...
		function postDel(data){
			var delData = data.map(p=>p[_columns.indexOf("GroupID")]);

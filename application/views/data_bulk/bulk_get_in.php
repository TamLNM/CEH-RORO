...
<div class="row">
	<div class="col-xl-12" style="font-size: 12px;">
		<div class="ibox collapsible-box">
			...
			<div class="ibox-head">
				<div class="ibox-title">HÀNG RỜI GET-IN</div>
			...
			<div class="row ibox-footer border-top-0">
				<div class="col-md-12 col-sm-12 col-xs-12 table-responsive" id="tableIn">
					<div class="row">
						...
						<div class="col-6">
							<table id="tblStockInBulk" class="table table-striped display nowrap" cellspacing="0">
								<thead>
								<tr>
									<th class="editor-cancel" col-name="STT">STT</th>
									<th col-name="EirNo">Số lệnh</th>
									<th col-name="Sequence">Sequence</th>
									<th col-name="CargoWeightGetIn">Trọng lượng xe vào</th>
									<th col-name="UnitID">Đơn vị tính</th>
									<th class="editor-cancel data-type-checkbox" col-name="IsFinish">Hoàn tất</th>
								</tr>
								</thead>
								<tbody>
								</tbody>
...
<script type="text/javascript">
	$(document).ready(function () {
		var _stockColumns	= ["STT", "rowguid", "BillOfLadingORBookingNo", "BillOfLading", "BookingNo", "CargoWeight", "UnitID", "CommodityDescription"],
			_stockInColumns = ["STT", "EirNo", "Sequence", "CargoWeightGetIn", "UnitID", "IsFinish"];
		...
		tblStockInBulk.newDataTable({
			scrollY: '45vh',
			columnDefs: [
				{ type: "num", className: "text-center", targets: _stockInColumns.getIndexs(["STT"]) },	
				{ className: "text-center", targets: _stockInColumns.getIndexs( ["EirNo", "Sequence", "CargoWeightGetIn", "UnitID", "IsFinish"] )},		
			],
			order: [[ _stockInColumns.indexOf('STT'), 'asc' ]],
			paging: false,
            keys:true,
            autoFill: {
                focus: 'focus'
            ...
            rowReorder: false,
            arrayColumns: _stockInColumns,
            buttons: [],
		});
		//tblStockInBulk.editableTableWidget({editor: $("#status, #httt, #editor-input")});
		...
		$("#nochooseVessel").on('click', function(){
			...
			tblStockInBulk.dataTable().fnClearTable();
		...
		$(document).on("dblclick", "#tblVessel tbody tr",  function(){
       	...
       	$(document).on("click", "#tblStockBulk td",  function(){
            var data 	 = tblStockBulk.getSelectedRows().data().toArray()[0],
            	...
            	formData = {
            		'action': 'view',
            		'child_action': 'loadStockInBulkList',
            		'StockRef': stockRef,	
            	};

            tblStockInBulk.waitingLoad();
            $.ajax({
				url: "<?=site_url(md5('DataBulk') . '/' . md5('dtBulkGetIn'));?>",
				dataType: 'json',
				data: formData,
				type: 'POST',
				success: function (data) {
					...
					if(data.list.length > 0) {
						for (i = 0; i < data.list.length; i++) {
							var rData = data.list[i], r = [];
							$.each(_stockInColumns, function(idx, colname){
								var val = "";
								switch(colname){
									case "STT": 
										val = i+1; 
										break;
									case "IsFinish":
										val = '<label class="checkbox checkbox-primary"><input id="isActiveChb" type="checkbox" value="0"><span class="input-span"></span></label>';
										if (rData[colname] == 1){
											val = '<label class="checkbox checkbox-primary"><input id="isActiveChb" type="checkbox" value="1"><span class="input-span"></span></label>';
										}
										break;
									case "UnitID":
				    ...
				    if(rows.length > 0){
						tblStockInBulk.dataTable().fnAddData(rows);
			    	}
				},
				error: function(err){
					tblStockInBulk.dataTable().fnClearTable();

...
<div class="row">
	<div class="col-xl-12" style="font-size: 12px;">
		<div class="ibox collapsible-box">
			...
			<div class="ibox-head">
				<div class="ibox-title">HÀNG NHẬP - MANIFEST</div>
				<div class="button-bar-group mr-3">			
					...
					<a 	class="btn btn-outline-dark btn-sm mr-1"
						id="Export" 
						title="Export">
						href="<?=site_url(md5('Data') . '/' . md5('createXLSForManifestExport'));?>"
				...
				<div class="col-md-12 col-sm-12 col-xs-12 table-responsive" id="tableIn">
					<table id="contenttable" class="table table-striped display nowrap" cellspacing="0">
						...
						<tr>
							...
							<th col-name="BillOfLading" id="BillOfLadingColumn" class="BillOfLading">Số vận đơn</th>		
							...
							<th col-name="CarWeight">Trọng lượng (kg)</th>	
							<th col-name="BrandID">Hãng xe</th>	
							<th col-name="CarTypeID">Loại xe</th>	
							<th col-name="EngineType">Loại động cơ</th>	<!-- Load when user choose CarTypeID -->
							<th col-name="Sequence">Sequence</th>
							<th col-name="CaseNo">Case No</th>	
							<th col-name="ModelName">Model Name</th>	
							<th col-name="ChassisNumber">Chassis Number</th>	
							<th col-name="EngineSerial">Engine Serial</th>	
							<th col-name="BodyColor">Màu xe</th>	
							<th col-name="Interier">Interier</th>	
							<th col-name="Option">Option</th>	
							<th col-name="VonNo">Số Von</th>							
				...
				<div class="col-md-12 col-sm-12 col-xs-12 table-responsive" id="tableOut">
					<table id="contenttable2" class="table table-striped display nowrap" cellspacing="0">
						...
						<tr>
							...
							<th col-name="rowguid">rowguid</th>
							<th col-name="VoyageKey">Voyage Key</th>
							<th col-name="BookingNo" id="BookingNoColumn" class="BookingNo">Số booking</th>			
							<th col-name="VINNo">Số VIN</th>
							<th col-name="JobModeID">Phương án</th>
							<th col-name="MethodID">Phương thức</th>
							<th col-name="POL">Cảng xếp</th>							
								...
								<?php foreach($vesselList as $item) {  ?>
									<?php if ($item['Status'] != 7){ ?>
										<tr>
											...
											<td><?=$item['InboundVoyage'];?></td>
											<td><?=$item['OutboundVoyage'];?></td>
											<td><?=$item['ETA'];?></td>
											<td><?=$item['ETD'];?></td>								
											<td><?=$item['Status'];?></td>								
										</tr>
...
<!-- Brand modal-->
<div class="modal fade" id="brand-modal" tabindex="-1" role="dialog" aria-labelledby="groups-modalLabel-2" aria-hidden="true" data-whatever="id" style="padding-left: 14%">
	<div class="modal-dialog" role="document" style="width: 400px!important">
		<div class="modal-content" style="border-radius: 4px">
			<div class="modal-header">
				<h5 class="modal-title text-primary" id="groups-modalLabel-2">DANH MỤC HÃNG XE</h5>
			</div>
...
<!-- Car type modal-->
<div class="modal fade" id="car-type-modal" tabindex="-1" role="dialog" aria-labelledby="groups-modalLabel-3" aria-hidden="true" data-whatever="id" style="padding-left: 14%">
	<div class="modal-dialog" role="document" style="width: 400px!important">
		<div class="modal-content" style="border-radius: 4px">
			<div class="modal-header">
				<h5 class="modal-title text-primary" id="groups-modalLabel-3">DANH MỤC LOẠI XE</h5>
			</div>
			<div class="modal-body">
				<table id="tblCarType" class="table table-striped display nowrap" cellspacing="0" style="width: 99.5%">
					<thead>
						<tr style="width: 100%">
							<th col-name="STT">STT</th>
							<th col-name="CarTypeID">Mã loại</th>
							<th col-name="CarTypeName">Tên loại</th>
							<th col-name="CarTypeEngine"></th>
						</tr>
					</thead>
					<tbody>
					<?php if(count($carTypeList) > 0) {$i = 1; ?>
			...
			<div class="modal-footer">
				<div  style="margin: 0 auto!important;">
					<button class="btn btn-sm btn-rounded btn-gradient-blue btn-labeled btn-labeled-left btn-icon" id="apply-car-type" data-dismiss="modal">
						<span class="btn-label"><i class="ti-check"></i></span>Xác nhận</button>
					<button class="btn btn-sm btn-rounded btn-gradient-peach btn-labeled btn-labeled-left btn-icon" data-dismiss="modal">
						<span class="btn-label"><i class="ti-close"></i></span>Đóng</button>
				</div>
...
<script type="text/javascript">
	$(document).ready(function () {
		var _columns 		= ["STT", "rowguid", "VoyageKey", "BillOfLading", "VINNo", "JobModeID", "MethodID", "POL", "POD", "FPOD", "ClassID", "IsLocalForeign", "CarWeight", "BrandID", "CarTypeID", "EngineType", "Sequence", "CaseNo", "ModelName", "ChassisNumber", "EngineSerial", "BodyColor", "Interier", "Option", "VonNo", "KeyNo", "Remark"],
			_columns2 		= ["STT", "rowguid", "VoyageKey", "BookingNo", "VINNo", "JobModeID", "MethodID", "POL", "POD", "FPOD", "ClassID", "IsLocalForeign", "CarWeight", "BrandID", "CarTypeID", "EngineType", "Sequence", "CaseNo", "ModelName", "ChassisNumber", "EngineSerial", "BodyColor", "Interier", "Option", "VonNo", "KeyNo", "Remark"], 
			...
			_carTypeColumns = ["STT", "CarTypeID", "CarTypeName", "CarTypeEngine"],
			tbl 	 		= $("#contenttable"),
			tbl2 			= $("#contenttable2"),
			tblVessel 		= $("#tblVessel"),
			tblBrand		= $("#tblBrand"),
			tblCarType		= $("#tblCarType"),
			portListByLane 	= {}
			jobModeList		= {},
			methodList		= {},
			brandList		= {},
			...
			parentMenuList 	= {},
			classList 		= {},
			jobModeListHasIsVessel = {},
			brandModal 		= $("#brand-modal"),
			carTypeModal 	= $("#car-type-modal");
		...
		<?php if(isset($jobModeListHasIsVessel) && count($jobModeListHasIsVessel) >= 0){?>
			jobModeListHasIsVessel = <?=json_encode($jobModeListHasIsVessel);?>;
		<?php } ?>

		/* Load data from Vessel table */
		...
		/* Load data from Method table */
		<?php if(isset($brandList) && count($brandList) >= 0){?>
			brandList = <?= json_encode($brandList);?>;
		<?php } ?>
		...
		tbl.newDataTable({
			...
			columnDefs: [
				...
				{ className: "hiden-input", targets: _columns.getIndexs(["VoyageKey", "rowguid"]) },		
				{ className: "text-center", targets: _columns.getIndexs( ["ClassID", "IsLocalForeign", "BillOfLading", "Sequence", "VINNo", "JobModeID", "MethodID", "BrandID", "CarTypeID", "EngineType", "CaseNo", "ModelName", "ChassisNumber", "EngineSerial", "BodyColor", "Interier", "Option", "VonNo", "CarWeight", "KeyNo", "POL", "POD", "FPOD", "Remark"] )},		
			],
		...
		tbl2.newDataTable({
			...
			columnDefs: [
				...
				{ className: "hiden-input", targets: _columns2.getIndexs(["VoyageKey", "rowguid"]) },		
				{ className: "text-center", targets: _columns2.getIndexs( ["ClassID", "IsLocalForeign", "BookingNo", "Sequence", "VINNo", "JobModeID", "MethodID", "BrandID", "CarTypeID", "EngineType", "CaseNo", "ModelName", "ChassisNumber", "EngineSerial", "BodyColor", "Interier", "Option", "VonNo", "CarWeight", "KeyNo", "POL", "POD", "FPOD", "Remark"] )},		
			],
		...
		/* Initial car type table */	
		tblCarType.newDataTable({
			scrollY: '30vh',
			columnDefs: [
				{ type: "num", className: "text-center", targets: _carTypeColumns.indexOf('STT')},		
				{ className: "text-center", targets: _carTypeColumns.getIndexs(["CarTypeID", "CarTypeName"])},
				{ className: "hiden-input", targets: _carTypeColumns.getIndexs(["CarTypeEngine"]) },
			],
			order: [[ _carTypeColumns.indexOf('STT'), 'asc' ]],
			paging: false,
            keys:true,
            autoFill: {
                focus: 'focus'
	    ...
	    /* Auto edit size for table in modal*/
		...
		/* Set dropdown list for Job-Mode-Out */
       	tbl.setExtendDropdown({
			...
			source: jobModeListHasIsVessel,
			colIndex: _columns.indexOf("JobModeID"), // ô cần show drop-down box
			onSelected: function(cell, value){ // Thao tác sau khi người dùng lựa chọn
				var JobModeName = jobModeListHasIsVessel.filter( p => p.JobModeID == value).map( x => x.JobModeName );
				tbl.DataTable().cell(cell).data(
		...
		/*** SET DROPDOWN LIST FOR TABLE IN TAB OUT ***/
	    /* Set dropdown list for ClassID */
		...
		/* Set dropdown list for Job-Mode-Out */
       	tbl2.setExtendDropdown({
			...
			source: jobModeListHasIsVessel,
			colIndex: _columns2.indexOf("JobModeID"), // ô cần show drop-down box
			onSelected: function(cell, value){ // Thao tác sau khi người dùng lựa chọn
				var JobModeName = jobModeListHasIsVessel.filter( p => p.JobModeID == value).map( x => x.JobModeName );
				tbl2.DataTable().cell(cell).data(
					'<input class="hiden-input" value="'+ value  +'">' + JobModeName
		...
		/* Set dropdown list for Method-In */
       	tbl2.setExtendDropdown({
			...
			source: methodList,
			colIndex: _columns2.indexOf("MethodID"), // ô cần show drop-down box
			onSelected: function(cell, value){ // Thao tác sau khi người dùng lựa chọn
				var MethodName = methodList.filter( p => p.MethodID == value).map( x => x.MethodName );
				tbl2.DataTable().cell(cell).data(
					'<input class="hiden-input" value="'+ value  +'">' + MethodName
		...
		/* Initial ETA, ETD datapicker */
		$('#ETA, #ETD').datetimepicker({
			controlType: 'select',
			oneLine: true,
			dateFormat: 'dd/mm/yy',
			timeFormat: 'HH:mm:00',
		...
		/* Child  table when choose Vessel */
       	$("#chooseVessel").on('click', function(){
			...
			/* Load data for vessel table (pop-up) */
			/*
			var rows = [];
			if (vesselList.length > 0){
				for (i = 0; i < vesselList.length; i++) {
					var rData = vesselList[i], r = [];
		...
		/* Refresh vessel list 
		$("#refreshVesselList").on('click', function(){
			$('input[value=VesselFilter]').attr('checked', 'checked').trigger('change');
		});
		*/
								...
								$.each(_columns, function(idx, colname){
									...
									switch(colname){
										...
										case "MethodID":
											...
											break;
										case "BrandID":
											val = (rData[colname] == null) ? '' : '<input class="hiden-input" value="' + rData[colname] + '">' + rData['BrandName'];
											break;
										case "CarTypeID":
											val = (rData[colname] == null) ? '' : '<input class="hiden-input" value="' + rData[colname] + '">' + rData['CarTypeName'];
											break;
										case "POL":
											val = (rData[colname] == null) ? '' : '<input class="hiden-input" value="' + rData[colname] + '">' + rData['POLName'];
											break;
										case "POD":
											val = (rData[colname] == null) ? '' : '<input class="hiden-input" value="' + rData[colname] + '">' + rData['PODName'];
											break;
										case "FPOD":
											val = (rData[colname] == null) ? '' : '<input class="hiden-input" value="' + rData[colname] + '">' + rData['FPODName'];
											break;
										default:
		...
		// Vessel filter radio change
		//$('input[type=radio][name=VesselFilter]').change(function() {
		...
		//tblVessel.find("tbody tr").on("dblclick", function(){ //
       	$(document).on("dblclick", "#tblVessel tbody tr",  function(){
			...
			$.ajax({
       		...
       		// Check value exists in tbl
       		if (($('input[type=radio][name=ClassID]:checked').val() == 1 && tbl.DataTable().rows( '.addnew, .editing' ).data().toArray().length > 0) || ($('input[type=radio][name=ClassID]:checked').val() == 2 && tbl2.DataTable().rows( '.addnew, .editing' ).data().toArray().length > 0)){
       			$.confirm({
		            title: 'Thông báo!',
		            type: 'orange',
		            icon: 'fa fa-warning',
		            content: 'Bạn có muốn hủy việc chỉnh sửa các dòng hiện tại?',
		            buttons: {
		                ok: {
		                    text: 'Chấp nhận',
		...
		$("#apply-vessel").on("click", function(){
       		...
       		var VoyageKey		= tblVesselSelectedRows[_vesselColumns.indexOf("VoyageKey")],
			...
			$.ajax({
       		...
       		if (($('input[type=radio][name=ClassID]:checked').val() == 1 && tbl.DataTable().rows( '.addnew, .editing' ).data().toArray().length > 0) 
       			|| ($('input[type=radio][name=ClassID]:checked').val() == 2 && tbl2.DataTable().rows( '.addnew, .editing' ).data().toArray().length > 0)){
       			$.confirm({
		            title: 'Thông báo!',
		            type: 'orange',
		            icon: 'fa fa-warning',
        	...
        	if (numRows == 1){ // Table In
        		if ($('input[type=radio][name=ClassID]:checked').val() == 1){
					...
					for (i = 0; i < newData.length; i++){
						...
						var cell = tbl2.find("tbody tr:eq(" + i + ") td:eq("+ _columns.indexOf("JobModeID") +")");					
						tbl2.DataTable().cell(cell).data(
							'<input class="hiden-input" value="XTAU">XUẤT TÀU'
						).draw(false);

						var cell = tbl2.find("tbody tr:eq(" + i + ") td:eq("+ _columns.indexOf("MethodID") +")");					
						tbl2.DataTable().cell(cell).data(
							'<input class="hiden-input" value="TAU-BAI">TÀU BÃI'
						).draw(false); 
					}
		...
		/* Prevent press '-' */
       	...
       	/* Child table when choose Brand cell*/
        // Set extension button choose Custom 
		tbl.setExtendSelect( _columns.indexOf("BrandID"), function(rIdx, cIdx){
			brandModal.val(rIdx + "." + cIdx);
			brandModal.modal("show");
		});
		tbl2.setExtendSelect( _columns.indexOf("BrandID"), function(rIdx, cIdx){
			brandModal.val(rIdx + "." + cIdx);
			brandModal.modal("show");
		});
		...
		$("#apply-brand").on("click", function(){	// Click Accept
			var rIdx = brandModal.val().split(".")[0],
				cIdx = brandModal.val().split(".")[1],
				a_cIdx = brandModal.val().split(".")[2],
		...
		/* Child table when choose Car Type cell*/
        // Set extension button choose Car Type 
		tbl.setExtendSelect( _columns.indexOf("CarTypeID"), function(rIdx, cIdx){
			a_cIdx = cIdx + 1;
			carTypeModal.val(rIdx + "." + cIdx + "." + a_cIdx);
			carTypeModal.modal("show");
		});
		tbl2.setExtendSelect( _columns.indexOf("CarTypeID"), function(rIdx, cIdx){
			a_cIdx = cIdx + 1;
			carTypeModal.val(rIdx + "." + cIdx + "." + a_cIdx);
			carTypeModal.modal("show");
		});
		...
		// Adjust column in table when show modal
		carTypeModal.on('shown.bs.modal', function(e){
			$($.fn.dataTable.tables(true)).DataTable().columns.adjust();
		});

		// Select brand on table
		tblCarType.find("tbody tr").on("dblclick", function(){ // Double Click
			var applyBtn = carTypeModal,
				rIdx = applyBtn.val().split(".")[0],
				cIdx = applyBtn.val().split(".")[1],
				a_cIdx = applyBtn.val().split(".")[2],
				carTypeID = $(this).find("td:eq("+_carTypeColumns.indexOf("CarTypeID")+")").text(),
				carTypeName  = $(this).find("td:eq("+_carTypeColumns.indexOf("CarTypeName")+")").text(),
				carTypeEngine = $(this).find("td:eq("+_carTypeColumns.indexOf("CarTypeEngine")+")").text();

			if ($("input[name='ClassID']:checked").val() == 1){
				var cell 		= tbl.find("tbody tr:eq("+rIdx+") td:eq("+cIdx+")").first(),
					cellNextCol = tbl.find("tbody tr:eq("+rIdx+") td:eq("+a_cIdx+")").first(),
					dtTbl		= tbl.DataTable();	
				dtTbl.cell(cell).data(
					'<input class="hiden-input" value="'+ carTypeID  +'">' + carTypeName
				).draw();

				dtTbl.cell(cellNextCol).data(carTypeEngine).draw();
				...
				var cell 		= tbl2.find("tbody tr:eq("+rIdx+") td:eq("+cIdx+")").first(),
					cellNextCol = tbl2.find("tbody tr:eq("+rIdx+") td:eq("+a_cIdx+")").first(),
					dtTbl		= tbl2.DataTable();	
				dtTbl.cell(cell).data(
					'<input class="hiden-input" value="'+ carTypeID  +'">' + carTypeName
				).draw();
		...
		$("#apply-car-type").on("click", function(){	// Click Accept
			var rIdx = carTypeModal.val().split(".")[0],
				cIdx = carTypeModal.val().split(".")[1],
				a_cIdx = carTypeModal.val().split(".")[2],
				carTypeID 	= tblCarType.getSelectedRows().data().toArray()[0][_carTypeColumns.indexOf("CarTypeID")],
				carTypeName	= tblCarType.getSelectedRows().data().toArray()[0][_carTypeColumns.indexOf("CarTypeName")],
				carTypeEngine = tblCarType.getSelectedRows().data().toArray()[0][_carTypeColumns.indexOf("CarTypeEngine")];
            	...
            	if(tbl.getAddNewData().length > 0 || tbl2.getAddNewData().length > 0){
            		...
            		for (i = 0; i < newData.length; i++){
						if (newData[i]['ClassID'] == 1){
							...
							var BookingNo = newData[i]["BookingNo"];
							/* ClassName = 'Xuất' */
							if (BookingNo == ''){
		...
		function saveData(){
			...
			if(newData.length > 0){
				/* Get save data for Stock table */
				if ($('input[type=radio][name=ClassID]:checked').val() == 1){
					var dataSave = []/*, dataJobQuaySave = []*/;
					for (i = 0; i < newData.length; i++){
						var obj = {
							'IsLocalForeign': 	newData[i]["IsLocalForeign"] ? newData[i]["IsLocalForeign"] : '',
							'ClassID': 			1,
							'BillOfLading': 	newData[i]["BillOfLading"] 	? newData[i]["BillOfLading"] 	: '',
							'Sequence': 		newData[i]["Sequence"] 		? newData[i]["Sequence"] 		: '',
							'VINNo': 			newData[i]["VINNo"] 		? newData[i]["VINNo"] 			: '',
							'POL': 				newData[i]["POL"] 			? newData[i]["POL"] 			: '',
							'POD': 				newData[i]["POD"] 			? newData[i]["POD"] 			: '',
							'FPOD': 			newData[i]["FPOD"] 			? newData[i]["FPOD"] 			: '',
							'Remark': 			newData[i]["Remark"]		? newData[i]["Remark"] 			: '',
							'VMStatus':			'B',
							'JobModeInID': 		newData[i]["JobModeID"]		? newData[i]["JobModeID"] 		: '',
							'MethodInID': 		newData[i]["MethodID"]		? newData[i]["MethodID"] 		: '',
						};
						dataSave.push(obj);					
		...
		$("#downloadFileForImport").on('click', function(){
			if ($("input[name='ClassID']:checked").val() == 1){
				var url = '<?=site_url(md5('Data') . '/' . md5('createXLSFormForManifestImportForClassIn'));?>';
				window.location.href = url;
			}
			else{
				var url = '<?=site_url(md5('Data') . '/' . md5('createXLSFormForManifestImportForClassOut'));?>';
				window.location.href = url;
			}
		});
						...
						$.each(_columns, function(idx, colname){
							...
							switch(colname){
								...
								case "IsLocalForeign":
									...
									break;
								case "BillOfLading":
									val = (rData['Số vận đơn/ Số booking'] ? rData['Số vận đơn/ Số booking'] : '');
									break;
								case "VINNo":
									val = (rData['Số VIN'] ? rData['Số VIN'] : '');
									break;
								case "JobModeID":
									val = (rData['Mã phương án (xem ở cột AB - AD)'] ? rData['Mã phương án (xem ở cột AB - AD)'] : '');
									break;
								case "MethodID":
									val = (rData['Mã phương thức (xem ở cột AF - AH)'] ? rData['Mã phương thức (xem ở cột AF - AH)'] : '');
									break;
								case "BrandID":
									val = (rData['Mã hãng xe (xem ở cột AJ - AL)'] ? rData['Mã hãng xe (xem ở cột AJ - AL)'] : '');
									break;
								case "CarTypeID":
									val = (rData['Mã loại xe (xem ở cột AN - AQ)'] ? rData['Mã loại xe (xem ở cột AN - AQ)'] : '');
									break;
								case "EngineType":
									val = (rData['Loại động cơ (xem ở cột AN - AQ)'] ? rData['Loại động cơ (xem ở cột AN - AQ)'] : '');
									break;
								case "CaseNo":
									val = (rData['Case No'] ? rData['Case No'] : '');
									break;	
								case "ModelName":
									val = (rData['Model Name'] ? rData['Model Name'] : '');
									break;	
								case "ChassisNumber":
									val = (rData['Chassic number'] ? rData['Chassic number'] : '');
									break;
								case "EngineSerial":
									val = (rData['Engine Serial'] ? rData['Engine Serial'] : '');
									break;	
								case "BodyColor":
									val = (rData['Màu xe'] ? rData['Màu xe'] : '');
									break;
								case "VonNo":
									val = (rData['Số Von'] ? rData['Số Von'] : '');
									break;
								case "CarWeight":
									val = (rData['Trọng lượng (kg)'] ? rData['Trọng lượng (kg)'] : '');
									break;
								case "KeyNo":
									val = (rData['Mã chìa khóa'] ? rData['Mã chìa khóa'] : '');
									break;
								case "POL":
									val = (rData['Cảng xếp'] ? rData['Cảng xếp'] : '');
									break;
								case "POD":
									val = (rData['Cảng dở'] ? rData['Cảng dở'] : '');
									break;
								case "FPOD":
									val = (rData['Cảng đích'] ? rData['Cảng đích'] : '');
									break;
								case "Remark":
									val = (rData['Ghi chú'] ? rData['Ghi chú'] : '');
									break;
								default:
									val = (rData[colname] ? rData[colname] : '');
									break;											
			...
			for (i = 0; i < excelRows.length; i++){
				...
				if (listAddEdit[i] == 0){
					if ($("input[name='ClassID']:checked").val() == 1){
						...
						tbl2.DataTable().rows(crRow).nodes().to$().addClass("addnew");
					}
				}	
				else{
					if ($("input[name='ClassID']:checked").val() == 1){
						...
						tbl2.DataTable().rows(crRow).nodes().to$().addClass("editing");
					}
				}
			}
	    };
	    ...
	    $("#Export").on('click', function(){
	    	if ($("input[name='ClassID']:checked").val() == 1){
	    		var url = "<?=site_url(md5('Data') . '/' . md5('createXLSForManifestExportWithClassIn'));?>";
				window.location.href = url;
	    	}
	    	else{
	    		var url = "<?=site_url(md5('Data') . '/' . md5('createXLSForManifestExportWithClassOut'));?>";
				window.location.href = url;
	    	}
	    });
	});

				...
				<div class="col-md-12 col-sm-12 col-xs-12 table-responsive">
					<div id="tablecontent">
						<table id="contenttable" class="table table-striped display table-bordered nowrap" cellspacing="0" style="width: 99.9%">
							...
							<tr>
								<td rowspan="2">STT</td>
								<td rowspan="2">DOANH THU LÀM THỦ TỤC DỊCH VỤ (TIỀN MẶT)</td>
								<td colspan="3">LOẠI CONTAINER</td>
								<td rowspan="2">TỔNG TIỀN</td>
							</tr>
							<tr>
								<td>20'</td>
								<td>40'</td>
				...
				success: function (data) {
					...
					if(data.results.length > 0) {
						...
						for (i = 0; i < data.results.length; i++) {
							rows.push([
								(i+1)
								, data.results[i].TRF_DESC
								, data.results[i]["20"]
								, data.results[i]["40"]
								, data.results[i]["45"]
								, data.results[i].SUMAMOUNT
							]);
						}
					}
					if(rows.length > 0){
						rows.push([
							  rows.length + 10
							, ''
							, data.results.map(p=> p["20"]).reduce(function(a, b) {return a+b;}, 0)
							, data.results.map(p=> p["40"]).reduce(function(a, b) {return a+b;}, 0)
							, data.results.map(p=> p["45"]).reduce(function(a, b) {return a+b;}, 0)
							, data.results.map(p=> p.SUMAMOUNT).reduce(function(a, b) {return a+b;}, 0)
						]);
					}
					...
					$('#contenttable').DataTable( {
						...
						columnDefs: [
							...
							{ className: "text-right", targets: [2,3,4], render: $.fn.dataTable.render.number( ',', '.', 0) }
						],
						order: [[ 0, 'asc' ]],
						scroller: {
							displayBuffer: 9,
